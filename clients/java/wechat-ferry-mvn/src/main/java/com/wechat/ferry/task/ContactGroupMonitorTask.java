...
public class ContactGroupMonitorTask {
    ...
    @PostConstruct
    public void showMsgType() {
        List<WxPpWcfMsgTypeResp> wxPpWcfMsgTypeResps = weChatDllService.queryMsgTypeList();
        for (WxPpWcfMsgTypeResp wxPpWcfMsgTypeResp : wxPpWcfMsgTypeResps) {
            log.info("[消息类型]- {} 对应 {}", wxPpWcfMsgTypeResp.getId(), wxPpWcfMsgTypeResp.getName());
        }
    }
    ...
    private void contactMonitor(List<ContactDo> contactList) {
        ...
        if (!CollectionUtils.isEmpty(contactList)) {
            ...
            if (initStatus) {
                ...
                log.info(
                        "[定时任务]-[联系人监控]-个微新增：{}，个微删除：{}，企微新增：{}，企微删除：{}，群组新增：{}，群组删除：{}",
                        addPpContactList, deletePpContactList, addCpContactList,
