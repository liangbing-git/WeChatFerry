...
public class WeChatMsgServiceImpl implements WeChatMsgService {
    ...
    public void receiveMsg(String jsonString) {
        ...
        if (!CollectionUtils.isEmpty(weChatFerryProperties.getOpenMsgGroups())) {
            if (weChatFerryProperties.getAdminUsers().contains(dto.getSender())) {
                // 管理员说的特殊指令
                ReceiveMsgFactory.getStrategy(ReceiveMsgChannelEnum.ADMIN_MSG.getCode()).doHandle(dto);
            }
            if (isGroupMsg(dto)) {
                ReceiveMsgFactory.getStrategy(ReceiveMsgChannelEnum.GROUP_MSG.getCode()).doHandle(dto);
            }
        }
        log.debug("[收到消息]-[消息内容]-打印：{}", dto);
    }

    private boolean isGroupMsg(WxPpMsgDTO dto) {
        return weChatFerryProperties.getOpenMsgGroups().contains(dto.getRoomId())
                || weChatFerryProperties.getOpenMsgGroups().contains("ALL");
    }


    private void receiveMsgCallback(String jsonString) {
        // 开启回调，且回调地址不为空
        if (weChatFerryProperties.getReceiveMsgCallbackSwitch() && !CollectionUtils.isEmpty(
                weChatFerryProperties.getReceiveMsgCallbackUrls())) {
    ...
    private Boolean judgeSuccess(String responseStr) {
        ...
        if (!ObjectUtils.isEmpty(responseStr)) {
            ...
            if (!ObjectUtils.isEmpty(jSONObject) && !CollectionUtils.isEmpty(
                    weChatFerryProperties.getThirdPartyOkCodes())) {
                Map<String, String> codeMap = weChatFerryProperties.getThirdPartyOkCodes();
                for (Map.Entry<String, String> entry : codeMap.entrySet()) {
                    if (!ObjectUtils.isEmpty(jSONObject.get(entry.getKey())) && jSONObject.get(entry.getKey()).equals(
                            entry.getValue())) {
                        passFlag = true;
                        break;
                    }
